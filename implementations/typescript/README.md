...
CI (`.github/workflows/typescript.yml`) runs install/build/typecheck/lint/test/format across the workspace. `@blockparty/sdk` is publish-ready (`pnpm --filter @blockparty/sdk publish`).

## Modules

Each `@blockparty/sdk` module mirrors the Go reference in [`../go`](../go) and
